(ns trimble-ag.main
  "Kotodama WASM entrypoint for the Trimble_ag clean-room actor (L4) — Clojure port.
  ...
  facts (`trimble_ag.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "trimble_ag")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"trimble_ag.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "trimble_ag-compat" :tier tier :entities entities} 200])
...
(defn start! [] :trimble_ag-compat/ready)
